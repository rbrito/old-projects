...
public class MipsFrame extends Frame.Frame {
    ...
    /*
     * A arquitetura MIPS possui um total de 32 registradores, numerados de 0 a
     * 31 e referenciados como $0 ate' $31. Entretanto, a especificacao da
     * plataforma tambem determina nomes para esses registradores, que podem ser
     * usados em lugar de seus numeros. Por exemplo, $zero em vez de $0. Abaixo
     * segue a lista de temporarios reservados para os registradores da
     * arquitetura.
     * 
     * Uma possivel mudanca na classe Temp.Temp que seria bem-vinda aqui seria a
     * adicao de mais um construtor que recebesse uma string como um parametro e
     * que, em vez de guardar t123, como a descricao interna do temporario
     * guardasse a string passada como parametro. Desta forma, poderiamos dar o
     * seguinte comando:
     * 
     * private final Temp zero = new Temp.Temp("$zero");
     * 
     * e o metodo tempMap simplesmente chamaria o metodo toString dos
     * temporarios (por exemplo, zero.toString()) em vez de fazer diversos if
     * para determinar o "nome em Assembly" do registrador zero.
     */
    ...
    private static final TempList specialregs = L(zero,
            L(at, L(v0, L(v1, L(k0, L(k1, L(gp, L(sp, L(fp, L(ra, null))))))))));

    // Lista de registradores de argumentos de funcoes: a[0-3]
    private static final TempList argregs = L(a0, L(a1, L(a2, L(a3, null))));

    // Lista de registradores callee-save: s[0-7]
    private static final TempList calleesaves = L(s0,
            L(s1, L(s2, L(s3, L(s4, L(s5, L(s6, L(s7, null))))))));

    // Lista de registradores caller-save: t[0-9]
    private static final TempList callersaves = L(t0,
            L(t1, L(t2, L(t3, L(t4, L(t5, L(t6, L(t7, L(t8, L(t9, null))))))))));

    // Lista de registradores modificados em chamadas de funcoes:
    // v0, v1, fp, ra, t[0-9]
    private static final TempList calldefs = L(ra,
            L(v0, L(v1, L(argregs, callersaves))));
    ...
    private static final TempList allregs = L(
            zero,
            L(at,
                    L(v0,
                            L(v1,
    ...
    public Tree.Stm procEntryExit1(Tree.Stm body) {
        ...
        // L(s7, L(s6, L(s5, L(s4, L(s3, L(s2, L(s1, L(s0, L(ra, L(a3, L(a2,
        // L(a1, L(a0, null)))))))))))));
        ...
        for (l = calleesaves2; l != null; l = l.tail) {
            ...
            body = new Tree.SEQ(new Tree.MOVE(
                    tail.head.exp(new Tree.TEMP(FP())), new Tree.TEMP(l.head)),
                    body);
        }

        // restaurando registradores calleesaves
        for (l = calleesaves2; l != null; l = l.tail) {
            body = new Tree.SEQ(body, new Tree.MOVE(new Tree.TEMP(l.head),
    ...
    public Assem.InstrList procEntryExit2(Assem.InstrList body) {
        return append(body, new Assem.InstrList(new Assem.OPER("", null,
                returnSink), null));
    }

    // Metodo dado pelo livro -- FIXME: o que e' Frame.Proc?
    public Frame.Proc procEntryExit3(Assem.InstrList body) {
        Assem.InstrList p = new Assem.InstrList(new Assem.LABEL(name.toString()
                + ":\n", name), new Assem.InstrList(new Assem.OPER(
