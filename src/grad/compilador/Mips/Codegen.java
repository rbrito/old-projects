...
public class Codegen {
	...
	if (dst.exp instanceof Tree.BINOP) {
		...
		emit(new Assem.OPER("\tsw `s1, " + c.value + "(`s0)\n",
				    null, L(t.temp, L(munchExp(src), null))));
		return;
	    }
	...
	Temp.LabelList list =
	    new Temp.LabelList(s.iftrue, new Temp.LabelList(s.iffalse, null));
	...
	if (e.left instanceof Tree.CONST) { // Caso o primeiro arg. seja constante
