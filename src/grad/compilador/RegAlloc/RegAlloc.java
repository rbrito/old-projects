 ...
 *   CLASSE RegAlloc                                                                                       *
 * ------------------------------------------------------------------------------------------------------- *
 *   Realiza a alocação de registradores para cada temporário. Para isso, faz uma K-coloração do grafo de  *
 *   interferências (onde K é o número de registradores disponíveis na máquina destino).                   *
 ***********************************************************************************************************/

public class RegAlloc implements TempMap
{
  private InterferenceGraph ig;   // grafo de interferências
  private int K;                  // número de registradores disponíveis (para fazer K-coloração)

  private Frame frame;            // frame
  private InstrList il;           // instruções assembly sendo analisadas

  // as seguintes estruturas de dados representarão, através de conjuntos e tabelas de hash, o grafo de
  // interferências (de agora em diante, grafo refere-se a todos estas estruturas e grafo de interferências
  // ao grafo gerado durante a análise de liveness)
  private Dictionary color;    // associa cada temporário a uma cor (registrador)
  private Dictionary alias;    // associa dois temporários em MOVES que serão eliminados
  private Dictionary degree;   // associa a cada temporário do grafo o seu grau
  private OrderedSet
    selectStack;   // pilha de temporários para futura coloração
  private TempSet 
  ...
  private PairOrderedSet
    ...
    adjSet;
  private Dictionary
    moveList,      // associa cada temporário ao conjunto de MOVES ao qual está relacionado
    adjList;       // associa cada temporário ao conjunto de temporários com os quais interfere

  /* Strring tempMap (Temp t)
   *    Procura o nome do registrador (cor) associado ao temporário t.
   */
  ...
  /* int Degree (Temp t)
   *    obtém o grau do temporário t.
   */
  private int Degree (Temp t) {
    if (precolored.has(t))
      return (1<<30);   // grau de temporários pré-coloridos é considerado infinito
    Integer i = (Integer)degree.get(t);
    return i.intValue();
  }

  /* PairOrderedSet moveList (Temp t)
   *    obtém o conjunto de MOVES associados ao temporário t.
   */
  private PairOrderedSet moveList (Temp t) {
    PairOrderedSet set = (PairOrderedSet) moveList.get(t);
    if (set == null) {
      set = new PairOrderedSet("moveList("+t+")");   set.showErro = false;
      moveList.put(t,set);
    }
    return set;
  }
   ...
   *    obtém o conjunto de temporários que interferem com o temporário t.
   */
   ...
   *    retorna o conjunto de instruções associado à funcão analisada.
   */
   ...
   *    realiza a alocação de registradores.
   */
  ...
  /* Initialize()
   *    inicializa os conjuntos a serem usados.
   */
  ...
  /* Main()
   *    função principal (recursiva até que não sejam criados mais temporário "spilled").
   */
  private void Main() {
    Liveness();
    Build();
  ...
  /* Liveness()
   *    faz o "liveness" estático, criando o grafo de interferências.
   */
  ...
  /* Build()
   *    A partir do grafo de interferências, cria os conjuntos que serão uma nova representarão deste grafo,
   *    com os quais o algoritmo de alocação de registradores trabalhará.
   */
  private void Build() {
    ...
    // coloca cada temporário no devido conjunto e cria os conjuntos representando as interferências.
    for (NodeList nl=ig.nodes(); nl!=null; nl=nl.tail) {
      Temp u = ig.gtemp(nl.head);
      if (!precolored.has(u))
	initial.add(u);
   ...
   *    cria uma aresta de interferência entre os temporários u e v.
   */
  ...
  /* MakeWorklist()
   *    transfere cada temporário do conjunto initial para o conjunto:
   *    - spillWorklist (se for de grau significante)
   *    - freezeWorklist (se for de grau não-significante, associado a MOVE)
   *    - simplifyWorklist (se for de grau não-significante, não associado a MOVE)
   */
   ...
   *    retorna o conjunto dos temporários que ainda interferem com o temporário n.
   */
   ...
   *    retorna o conjunto dos MOVES associados ao temporário n.
   */
   ...
   *    verifica se o temporário n está associado a algum MOVE.
   */
  ...
  /* Simplify()
   *    escolhe um dos temporários de simplifyWorklist, remove-o do grafo e o coloca na pilha para futura
   *    coloração.
   */
  private void Simplify() {
    ...
    for (Temp m=(Temp)adj.getFirst(); m!=null; m=(Temp)adj.getNext(m))
      DecrementDegree(m);
  }
    
  /* DecrementDegree()
   *    após remover um temporário do grafo, atualiza seus vizinhos.
   */
   ...
   *    habilita os MOVES associados aos nós presentes no conjunto nodes.
   */
  private void EnableMoves (TempSet nodes) {
    for (Temp n=(Temp)nodes.getFirst(); n!=null; n=(Temp)nodes.getNext(n)) {
      ...
      for (Pair m=(Pair)nm.getFirst(); m!=null; m=(Pair)nm.getNext(m.x,m.y)) {
  ...
  /* Coalesce()
   *    já que há MOVES em worklistMoves, escolhe um deles e tenta eliminar o MOVE fazendo com que o
   *    temporário destino fique no mesmo registrador que o temporário origem.
   */
  private void Coalesce() {
    ...
    if (u == v) {
      ...
      // este MOVE não pode ser eliminado: os temporários do MOVE interferem
  ...
  /* AddWorklist (Temp u)
   *    o temporário u será marcado para simplificar (não será mais considerado para "coalesce")
   */
   ...
   *    verifica se o critério de George é satisfeito, ié, se para cada vizinho z de v, z já interefere com
   *    r ou z é de grau não-significante.
   */
  private boolean AdjacentOK (Temp v, Temp r) {
    boolean ok = true;
    TempSet adj = Adjacent(v);
    for (Temp t=(Temp)adj.getFirst(); t!=null && ok; t=(Temp)adj.getNext(t))
      ok = ( (Degree(t) < K) || precolored.has(t) || adjSet.has(t,r) );
    return ok;
  }

  /* Conservative (TempSet set)
   *    verifica se o critério de Briggs é satisfeito, ié, a partir de um conjunto com os vizinhos de dois
   *    temporários de um MOVE que será eliminado, o conjunto possui menos do que K temporários de grau
   *    significante.
   */
  private boolean Conservative (TempSet set) {
    int k = 0;
    for (Temp n=(Temp)set.getFirst(); n!=null; n=(Temp)set.getNext(n)) {
      if (Degree(n) >= K)
	k++;
    }
    return (k < K);
   ...
   *    retorna o alias de um temporário, ié, se o temporário pertencia a um MOVE que foi eliminado, retorna
   *    o outro temporário do MOVE.
   */
   ...
   *    realiza os preparativos para eliminar o MOVE ao qual dois temporários u e v estão associados.
   */
  private void Combine (Temp u, Temp v) {
    ...
    TempSet adj = Adjacent(v);
    for (Temp t=(Temp)adj.getFirst(); t!=null; t=(Temp)adj.getNext(t)) {
      int d = Degree(t);
      AddEdge(t,u);
      if ((d==K-1) && (Degree(t)==K))
  ...
  /* Freeze()
   *    dado que não há mais temporários para simplificar nem MOVES que podem ser eliminados, desiste de
   *    eliminar MOVES associado a algum temporário para tentar fazer futuras simplificações.
   */
  private void Freeze() {
  ...
  /* FreezeMoves (Temp u)
   *    desiste de eliminar MOVES associado ao temporário u.
   */
  ...
  /* SelectSpill()
   *    Como não puderam ser feitas simplificações, nem eliminações de MOVES, nem congelamento de MOVES,
   *    escolhe um temporário para um possível "spill" (lançamento no frame) para poder continuar fazendo
   *    simplificações.
   */
  private void SelectSpill() {
    // evita escolher os temporários que foram criados para fazer spill de outros temporários
  ...
  /* AssignColors()
   *    uma vez removidos todos os nós e colocados na pilha, remove cada temporário da pilha a medida que
   *    lhe atribui uma cor; se não houver cor, então joga-o para o frame ("actual spill").
   */
  private void AssignColors() {
    while (!selectStack.isEmpty()) {
      ...
      // verifica quais as cores que não poderão ser usadas para o temporário
    ...
    for (Temp n=(Temp)coalescedNodes.getFirst(); n!=null; n=(Temp)coalescedNodes.getNext(n))
      color.put(n, (Temp)color.get(GetAlias(n)));
  }

  /* RewriteProgram()
   *    reescreve o programa, gerando as instruções para spill se houver temporário marcados para spill.
   */
  private void RewriteProgram() {
    // o hashtable a seguir será usado para associar temporários que não poderão ficar em registrador
   ...
   *    gera as instruções necessário para realizar spill se houver temporários nesta instrução marcados
   *    para spill.
   */
  private InstrList replaceSpilledTemps (Instr i, Dictionary spilledLocations) {
    ...
    // substitui cada temporário t usado na instrução i marcado para "spill" por um novo temporário c,
	...
	if (auxSet.isEmpty()) {
	  ...
	  // aproveita o temporário c da instrução c <= M[loc] gerada anteriormente
	  newTemp = (Temp)auxSet.getFirst();
	// substitui o temporário "spilled" pelo novo que receberá o valor do frame
	if (i instanceof Assem.MOVE)
	  ((Assem.MOVE)i).src = newTemp;
	else
	  use.head = newTemp;
	// verifica se a instrução c <= M[loc] foi marcada para ser gerada
	if (!createFetchInstr)
	  continue;
	// gera as instruções para ler o valor armazenado no frame e colocá-lo no novo temporário
    ...
    // acrescenta a instrução assembly i, após todas as geradas para ler valores do frame de temporários 
    // "spilled"
    if (last == null)
      last = first = new Assem.InstrList(i,null);
    else
      last = last.tail = new Assem.InstrList(i,null);
    // substitui cada temporário t definido na instrução i e marcado para "spill" por um novo temporário c,
	...
	if (i instanceof Assem.MOVE)
	  ...
	  def.head = newTemp;
	Tree.Stm storeStm = new Tree.MOVE(frameAddress.exp(new Tree.TEMP(frame.FP())),
   ...
   *    tenta eliminar instruções inúteis, como MOVE a,a ou JUMP para label que vem logo a seguir;
   *    retorna true se conseguiu eliminar alguma instrução, false caso contrário.
   */
  private boolean RemoveUselessInstructions() {
	...
	if (p.head instanceof MOVE) {
	  ...
	  // desvio seguido de um rótulo; verifica se o desvio é feito para o rótulo a seguir; caso
   ...
   *    tenta remover labels que não são usados e substitui várias declarações de labels contíguas por uma
   *    só.
   */
  private boolean RemoveUnusedLabels() {
	...
	if (p.head.jumps() != null) {
	    ...
	    if (numberOfUses == null)
	      ...
	      numberOfUses = new Integer(numberOfUses.intValue()+1);
	    labelUse.put(l.head,numberOfUses);
	  }
	}
      }
    }
    // passo 2: caso haja várias declarações de labels contíguas, escolhe um dos rótulos como representante
    Label nextLabel;
    ...
    for (InstrList p=il; p!= null; p=p.tail) {
      ...
      if (p.head instanceof Assem.LABEL)
	numberOfUse = (Integer) labelUse.get(((Assem.LABEL)p.head).label);
      else if (p.head.jumps() != null) {
	for (LabelList targets = p.head.jumps().labels; targets!=null; targets=targets.tail) {
	  OrderedSet labelSet = (OrderedSet) declaredLabel.get(targets.head);
	  if (labelSet != null)
  ...
  /* ======= FUNÇÕES DE DEBUG (PARA VERIFICAR ERROS NA IMPLEMENTAÇÃO DA ALOCAÇÃO DE REGISTRADORES) ======= */

  /* TestInvariants()
   *    verifica se as 4 afirmativas (pág.254 do livro) são satisfeitas.
   */
  private void TestInvariants() {
	...
	//System.out.print("\tDegree: ");
      boolean test = true;
      TempSet firstSet = simplifyWorklist.union(freezeWorklist).union(spillWorklist);
      TempSet secondSet = firstSet.union(precolored);
      for (Temp u=(Temp)firstSet.getFirst(); u!=null; u=(Temp)firstSet.getNext(u)) {
	...
	//System.out.print("\tSimplify worklist: ");
      boolean test = true;
      for (Temp u=(Temp)simplifyWorklist.getFirst(); u!=null; u=(Temp)simplifyWorklist.getNext(u)) {
	...
	//System.out.print("\tFreeze worklist: ");
      boolean test = true;
      for (Temp u=(Temp)freezeWorklist.getFirst(); u!=null; u=(Temp)freezeWorklist.getNext(u)) {
	PairOrderedSet aux = moveList(u).makeCopy().intersection(moves);
	test = (test && ( (Degree(u)<K) && (!aux.isEmpty()) ) );
      }
      //System.out.println(test);
	...
	//System.out.print("\tSpill worklist: ");
      boolean test = true;
      for (Temp u=(Temp)spillWorklist.getFirst(); u!=null; u=(Temp)spillWorklist.getNext(u))
	test = (test && (Degree(u)>=K) );
      //System.out.println(test);
      tudoOk = tudoOk & test;
    }
    if (!tudoOk) System.out.println("ERRO: INVARIANTES NÃO PRESERVADAS");

  }

  /* showProgram()
   *    mostra as instruções assembly.
   */
  ...
  /* showSets()
   *    mostra os elementos presentes em cada conjunto de sets.
   */
  private void showSets() {
    ...
    String[] setsName={"precolored","initial","simplifyWorklist","freezeWorklist","spillWorklist",
  ...
  /* LookFor (Temp n)
   *    procura o temporário n entre os conjuntos presentes em sets.
   */
  private void LookFor (Temp n) {
    ...
    for (int i=0; i<sets.length; i++)
      if (sets[i].has(n)) System.out.print(setsName[i]+" ");
    System.out.println();
  }
}
