 ...
 *   CLASSE Liveness                                                                                       *
 * ------------------------------------------------------------------------------------------------------- *
 *   Faz análise do liveness de cada temporário do programa e constrói um grafo de interferências (este    *
 *   grafo possui uma aresta entre cada par de temporários que estão vivos no mesmo momento, indicando     *
 *   que não podem estar no mesmo registrador.                                                             *
 ***********************************************************************************************************/

public class Liveness extends InterferenceGraph
{
  // armazena todos os temporários que estão vivos após o nó (instrução assembly) associado (live-out)
  ...
  // associa a cada temporário o número de vezes em que é usado em todo o programa
   ...
   *    realiza a análise do liveness e a contrução do grafo de interferências a partir do grafo de fluxo
   *    do programa.
   */
   ...
   *    faz a análise do liveness de cada temporário a partir do grafo de fluxo do programa; como resultado,
   *    indica, após cada instrução assembly, quais os temporários que estão vivos.
   */
  private void makeLiveMap (AssemFlowGraph flow) {
    // armazena todos os temporários que estão vivos ao chegar a um nó (live-in)
    java.util.Dictionary liveInMap = new java.util.Hashtable();
    // associa a cada instrução assembly o conjunto dos temporários que são usados e os que são definidos
    java.util.Dictionary uses = new java.util.Hashtable();
    java.util.Dictionary defs = new java.util.Hashtable();
    
    // para cada instrução assembly, cria um TempSet com os temporários que são usados e outro com os que
    // são definidos
    ...
    boolean changed;    // indica se houve mudança nos conjuntos live-in e/ou live-out do algum nó
    NodeList flowNodes = revert(flow.nodes());   // "inverte" os nós do grafo de fluxo para acelerar o
	...
	Node n = nodes.head;
	// obtém os conjuntos com os temporários que estão vivos ao entrar e ao sair da instrução assembly
	...
	// atualiza o conjunto dos temporários que estão vivos ao sair deste nó
	...
	// atualiza o conjunto dos temporários que estão vivos ao chegar a este nó
   ...
   *    com as informações sobre o liveness de cada temporário e a partir do grafo de fluxo do programa,
  ...
  private void buildInterferenceGraph (AssemFlowGraph flow) {
	...
	// Para cada MOVE a <- c,  onde os temporários b1 ... bj estão live-out, será gerada uma aresta
	// de interferência (a,b1) .. (a,bj) para todo bk que não seja o mesmo que a.
	...
	// Para cada operação que não for MOVE que defina um temporário a, cujos temporários live-out sejam
	// b1 ... bj, será gerada uma aresta de interferência (a,b1) ... (a,bj)
   ...
   *    conta o número de vezez em que cada temporário é utilizado em todo o programa.
   */
   ...
   *    devolve o nó associado a um temporário no grafo de interferências.
   */
   ...
   *    devolve o temporário associado a um nó do grafo de interferências.
   */
   ...
   *    grava em out uma representação deste grafo de interferências.
   */
   ...
   *    retorna o custo associado a fazer "spill" do nó (temporário) fornecido.
   */
  public int spillCost (Node n) {
    // leva em conta o número de vezes que o temporário é usado e o grau deste temporário no grafo de
    // interferências
   ...
   *    retorna o número de vezes que um temporário é usado.
   */
