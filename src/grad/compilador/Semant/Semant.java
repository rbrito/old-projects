...
public class Semant { // unica classe publica no pacote Semant
	...
	case Absyn.OpExp.GE:
	    ...
	    if (!((aux1t == env.INT && aux2t == env.INT) ||
		  (aux1t == env.STRING && aux2t == env.STRING))) {
	    }
	...
	// tem o mesmo tipo que o parametro da funcao e devolve o tipo de retorno
	// da funcao
	...
	if (s instanceof FunEntry) {
		...
		if ((aux.ty.actual() == esp.fieldType.actual()) ||
		    (aux.ty.actual() == env.NIL &&
		     esp.fieldType.actual() instanceof RECORD)) {
		    args = args.tail; // processa proximo argumento
		    esp = esp.tail;
		} 
		else {
		    env.errorMsg.error(e.pos, "Erro: tipo incorreto de argumento " +
				       "a funcao.");
		    return new ExpTy(trans.nilExp(), env.INT);
		}
	    }
	    
	    if ((args == null) && (esp == null)) { // numero correto de argumentos
		return new ExpTy(trans.callExp(((FunEntry) s).label, elist, 
					       level), ((FunEntry) s).result);
	    }
	    else { // alguma lista acabou antes da outra
		env.errorMsg.error(e.pos, "Erro: numero incorreto de argumentos.");
		return new ExpTy(trans.nilExp(), env.INT);
	    }
	} 
	else { // s nao e' entrada de funcao na tabela de simbolos
	    if (s instanceof VarEntry) {
		env.errorMsg.error(e.pos, "Erro: variavel <" + e.func.toString() +
				   "> usada como funcao.");
	    }
	    else {
	...
	// verificamos se cada expressao tem o mesmo tipo que o campo do registro
	while ((f != null) && (taux != null)) {
	    et = transExp(f.init, breakl); // traduzimos a exp. de inicializacao
	    
	    if ((et.ty).actual() != taux.fieldType.actual() &&
		(taux.fieldType.actual() instanceof Types.RECORD &&
		 (et.ty).actual() != env.NIL)) {
		env.errorMsg.error(e.pos, "Erro: expressao de tipo incompativel " +
	...
	if ((init.ty.actual() != tipoElemento.actual()) &&
	    !(init.ty.actual() instanceof Types.NIL &&
	      tipoElemento.actual() instanceof Types.RECORD)) {
	...
	if (var.ty.actual() != expr.ty.actual() &&
	    !(expr.ty.actual() instanceof Types.NIL &&
	      var.ty.actual() instanceof Types.RECORD)) {
	...
	if (e.elseclause == null) {
		...
		env.errorMsg.error(e.pos, "Erro: if-exp nao pode produzir valor.");
		...
		!(et1.ty.actual() instanceof Types.NIL &&
		  et2.ty.actual() instanceof Types.RECORD) &&
		!(et1.ty.actual() instanceof Types.RECORD &&
		  et2.ty.actual() instanceof Types.NIL)) {
	...
	Absyn.Exp varexp = new Absyn.VarExp(e.pos,
					    new Absyn.SimpleVar(e.pos, e.var.name));
	
 	// novo while
	Absyn.Exp newtest = new Absyn.OpExp(e.pos, varexp, Absyn.OpExp.LE, e.hi);

	Absyn.ExpList newbody = new Absyn.ExpList(e.body, 
				  new Absyn.ExpList(
				    new Absyn.AssignExp(e.pos, new Absyn.SimpleVar(e.pos, e.var.name),
				     new Absyn.OpExp(e.pos, varexp, Absyn.OpExp.PLUS,
						 new Absyn.IntExp(e.pos, 1))),
				    null));
	...
	for (Absyn.DecList p = e.decs; p != null; p = p.tail) {
	    ...
	    if (aux == null)
		throw new Error("Erro interno: aux == null em transExp(LetExp).");
	...
	if (e instanceof Absyn.VarExp)    return transExp((Absyn.VarExp) e, breakl);
	if (e instanceof Absyn.NilExp)    return transExp((Absyn.NilExp) e, breakl);
	if (e instanceof Absyn.IntExp)    return transExp((Absyn.IntExp) e, breakl);
	if (e instanceof Absyn.StringExp) return transExp((Absyn.StringExp) e, breakl);
	if (e instanceof Absyn.CallExp)   return transExp((Absyn.CallExp) e, breakl);
	if (e instanceof Absyn.OpExp)     return transExp((Absyn.OpExp) e, breakl);
	if (e instanceof Absyn.RecordExp) return transExp((Absyn.RecordExp) e, breakl);
	if (e instanceof Absyn.SeqExp)    return transExp((Absyn.SeqExp) e, breakl);
	if (e instanceof Absyn.AssignExp) return transExp((Absyn.AssignExp) e, breakl);
	if (e instanceof Absyn.IfExp)     return transExp((Absyn.IfExp) e, breakl);
	if (e instanceof Absyn.WhileExp)  return transExp((Absyn.WhileExp) e, breakl);
	if (e instanceof Absyn.ForExp)    return transExp((Absyn.ForExp) e, breakl);
	if (e instanceof Absyn.BreakExp)  return transExp((Absyn.BreakExp) e, breakl);
	if (e instanceof Absyn.LetExp)    return transExp((Absyn.LetExp) e, breakl);
	if (e instanceof Absyn.ArrayExp)  return transExp((Absyn.ArrayExp) e, breakl);
	...
	if (v instanceof Absyn.SimpleVar)    return transVar((Absyn.SimpleVar) v);
	...
	// verificamos se o tipo de entrada e' de uma variavel (pode ser de funcao)
	...
	if (et.ty instanceof Types.ARRAY) {
		...
		env.errorMsg.error(v.pos, "Erro: expressao inteira obrigatoria.");
		return new ExpTy(trans.nilExp(), env.INT);
	    }
	...
	if (d.typ == null) { // declaracao com tipo implicito
		...
		env.errorMsg.error(d.pos, "Erro: expressao com valor <nil> usada " +
		...
		env.errorMsg.error(d.pos, "Erro: o tipo <" + d.typ.name.toString() +
				   "> de <" + d.name.toString() +
				   "> contem ciclo.");
		return null; // para nao continuar definindo variavel
	    }
	    
	    if ((init.ty.actual() == env.NIL) && !(tipo.actual() instanceof
						   Types.RECORD)) {
		...
		v = new VarEntry(level.allocLocal(d.escape),
				 tipo.actual());
		env.venv.put(d.name, v);
		return trans.varAlloc(v.access, init.exp, level);
	    }
	...
	while (f != null) {
		...
		if (body.ty.actual() != result.actual()) {
		    env.errorMsg.error(f.pos, "Erro: Tipos declarado e devolvido" +
	...
	if (((Types.NAME)env.tenv.get(d.name)).isLoop()) {
	    env.errorMsg.error(d.pos, "Advertencia: Definicao de <" +
